...
//
function addParticipant($params, $body)
{
    ...
    $apiUrl = APIURLBND;
    $apiUrlRequest = $apiUrl . '/group/inviteuser?key=' . trim($SessionName);
    //
    $adv = adv::conectar(Conexao::conectar());
    $validateToken = $adv->validateToken($SessionName);
    $whatsnotify = $validateToken->whatsnotify ?? $validateToken->userconnected;
    //
            ...
            foreach ($resData['data'] as $item) {
                ...
                if ($item['status'] !== "200" && $item['status'] !== "201" && $item['status'] !== "202") {
                    $results[] = [
                        "error" => true,
                        "participant" => $jid,
                        "message" => "Não foi possível adicionar participante ao grupo"
                    ];
                } else {
                    $results[] = [
                        "error" => false,
...
//
$router->post('/addParticipant', function ($params, $body) {
    addParticipant($params, $body);
});
