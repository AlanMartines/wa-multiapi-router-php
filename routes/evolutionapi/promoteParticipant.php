...
//
function promoteParticipant($params, $body)
{
    ...
    $apiUrl = APIURLBND;
    $apiUrlRequest = $apiUrl . '/group/makeadmin?key=' . trim($SessionName);
    //
            ...
            foreach ($resData['data'] as $item) {
                ...
                if ($item['status'] !== "200" && $item['status'] !== "201" && $item['status'] !== "202") {
                    $results[] = [
                        "error" => true,
                        "participant" => $jid,
                        "message" => "Não foi possível promover participante"
                    ];
                } else {
                    $results[] = [
                        "error" => false,
                        "participant" => $jid,
                        "message" => "Participante promovido com sucesso"
                    ];
                }
            }
        } else {
...
//
$router->post('/promoteParticipant', function ($params, $body) {
    promoteParticipant($params, $body);
});
