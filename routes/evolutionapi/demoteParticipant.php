...
//
function demoteParticipant($params, $body)
{
    ...
    $apiUrl = APIURLBND;
    $apiUrlRequest = $apiUrl . '/group/demoteadmin?key=' . trim($SessionName);
    //
            ...
            foreach ($resData['data'] as $item) {
                ...
                if ($item['status'] !== "200" && $item['status'] !== "201" && $item['status'] !== "202") {
                    $results[] = [
                        "error" => true,
                        "participant" => $jid,
                        "message" => "Não foi possível rebaixar participante"
                    ];
                } else {
                    $results[] = [
                        "error" => false,
                        "participant" => $jid,
                        "message" => "Participante rebaixado com sucesso"
                    ];
                }
            }
        } else {
...
//
$router->post('/demoteParticipant', function ($params, $body) {
    demoteParticipant($params, $body);
});
