...
//
function setProfileName($params, $body)
{
    ...
    $apiUrl = APIURLBND;
    $apiUrlRequest = $apiUrl . '/chat/updateProfileName/' . trim($SessionName);
    //
    if (!isset($body->profileName) || empty(trim($body->profileName))) {
        ...
        echo json_encode([
            "Status" => [
                ...
                "message" => "É necessário informar 'profileName'. Corrija e tente novamente."
            ]
        ]);
        exit;
    }
...
//
$router->post('/setProfileName', function ($params, $body) {
    setProfileName($params, $body);
});
