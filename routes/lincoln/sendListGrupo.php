...
//
function sendListGrupo($params, $body)
{
    checkParameter::sendList($params, $body);
    checkToken::validateToken($params, $body);
    $SessionName = $body->SessionName ?? null;
    $groupId = $body->groupId ?? null;
    $listMessage = $body->listMessage ?? null;
    $apiUrl = APIURLBND;
    $apiUrlRequest = $apiUrl . '/message/list?key=' . trim($SessionName);
    //
    $config = [
...
//
$router->post('/sendListGrupo', function ($params, $body) {
    sendListGrupo($params, $body);
});
