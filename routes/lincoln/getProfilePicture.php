...
//
function getProfilePicture($params, $body) {
    ...
    $apiUrl = APIURLBND;
    $apiUrlRequest = $apiUrl.'/misc/downProfile?key='.trim($SessionName);
    //
    $config = [
    ...
    if ($response === false) {
        ...
        echo json_encode([
            "Status" => [
                ...
                "message" => "Não foi possível obter picture"
            ]
        ]);
        exit;
    }
    ...
    if (!empty($responseData->error)) {
        ...
        $result = [
            "error" => false,
            "statusCode" => $httpStatus,
            "profilePicture" => $responseData->data,
            "message" => "Picture obtido com sucesso"
        ];
    }
...
//
$router->post('/GetProfilePicture', function ($params, $body) {
    getProfilePicture($params, $body);
});
//
$router->post('/getProfilePicture', function ($params, $body) {
    getProfilePicture($params, $body);
