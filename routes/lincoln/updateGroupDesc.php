...
//
function updateGroupDesc($params, $body)
{
    ...
    $desc = $body->desc ?? null;
    $apiUrl = APIURLBND;
    $apiUrlRequest = $apiUrl . '/group/updatedescription?key=' . trim($SessionName);
    //
    if (!isset($body->desc) || empty(trim($body->desc))) {
        ...
        echo json_encode([
            "Status" => [
                ...
                "message" => "É necessário informar 'desc'. Corrija e tente novamente."
            ]
        ]);
        exit;
    }
...
//
$router->post('/updateGroupDesc', function ($params, $body) {
    updateGroupDesc($params, $body);
});
