...
function getCode($params, $body)
{
    ...
    $apiUrl = APIURLBND;
    $apiUrlRequest = $apiUrl . '/instance/getcode?key=' . trim($SessionName);
    //
    $config = [
    ...
    if ($response === false) {
        ...
        echo json_encode([
            "Status" => [
                ...
                "message" => "Não foi possível obter o codigo de paremento"
            ]
        ]);
        exit;
    }
...
//
$router->post('/getCode', function ($params, $body) {
    getCode($params, $body);
});
